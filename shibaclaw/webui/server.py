...
def create_app(
    ...
    async def _ensure_agent():
        ...
        # If we need to create the agent, reload config fresh from disk/memory
        if agent is None:
            _load_latest_config()
        ...
        # Cancel existing background tasks if any
        nonlocal _bg_tasks
        for t in _bg_tasks:
            t.cancel()
        _bg_tasks.clear()

        # Start the background event loop for the agent to process system/cron messages
        task1 = asyncio.create_task(agent.run())

        # Start a consumer for outbound messages generated by background tasks (e.g. subagents)
