...
class ShibaBrain:
    """
    The core hunting loop of the Shiba.

    Coordinates between the ScentBuilder (context), the Thinker (LLM),
    the SkillVault (tools), and the PackMemory (history).
    """
    ...
    def __init__(
        ...
        logger.info("ShibaBrain initialized for workspace: {}", workspace)
    ...
    async def run(self) -> None:
        ...
        logger.info("🐕 Shiba is alert and ready to hunt!")
        ...
        while self._running:
            try:
                ...
                logger.warning("🐕 Scent trail lost while consuming inbound message: {}, continuing...", e)
                continue
    ...
    async def _dispatch(self, msg: InboundMessage) -> None:
        ...
        async with self._processing_lock:
            try:
                ...
                logger.info("🐕 Hunt halted for session {}", msg.session_key)
                raise
            except Exception:
    ...
    async def _process_message(
        ...
        logger.info("Following the traccia in {}:{} for session {}: {}", msg.channel, msg.sender_id, session_key, preview)
