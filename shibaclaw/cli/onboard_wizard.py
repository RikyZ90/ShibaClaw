"""Interactive onboarding questionnaire for shibaclaw."""
...
from shibaclaw.cli.model_info import (
    format_token_count,
...
def _show_config_panel(display_name: str, model: BaseModel, fields: list) -> None:
    ...
    table.add_column("Field", style="gold1")
    ...
    console.print(Panel(table, title=f"[bold]{display_name}[/bold]", border_style="orange3"))
...
def _show_main_menu_header() -> None:
    ...
    console.print(
        Align.center(f"{__logo__} [bold gold1]shibaclaw[{__version__}][/bold gold1]")
    )
    console.print(Align.center("[dim]The ultimate companion for your AI hunt[/dim]"))
    console.print()
...
def _show_welcome_splash() -> None:
    ...
    console.print("\n" * 2)
    console.print(Panel(
        f"[bold gold1]{__logo__} Welcome to ShibaClaw![/bold gold1]\n\n"
        "I'll help you set up your kennel so you can start hunting with AI.\n"
        "We'll go through [bold]Foundation[/bold], [bold]AI Providers[/bold], and [bold]Integrations[/bold].",
        border_style="orange3",
        padding=(1, 2)
    ))
    console.print("\n[dim]Press any key to jump into the hunt...[/dim]")
    _get_questionary().press_any_key_to_continue().ask()


def _show_success_splash(config_path: Any) -> None:
    ...
    console.print(Panel(
        ...
        f"Your configuration has been saved at: [cyan]{config_path}[/cyan]\n"
        "ShibaClaw is ready to protect the kennel and hunt for answers.",
        title=f"{__logo__} [bold gold1]Good Boy![/bold gold1]",
        border_style="green",
        padding=(1, 2)
    ))
    console.print("\n[bold]Next Steps:[/bold]")
    console.print("  • Run [cyan]shibaclaw agent -m 'Hello'[/cyan] to start a chat.")
    console.print("  • Run [cyan]shibaclaw gateway[/cyan] to enable Telegram/WhatsApp.\n")
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from shibaclaw.config.schema to get
    ...
    from shibaclaw.config.schema import AgentDefaults
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"shibaclaw.integrations.{name}")
...
def run_onboard(initial_config: Config | None = None) -> OnboardResult:
    ...
    # If it's a fresh start, offer the Guided Wizard
    if is_fresh:
        _show_welcome_splash()
        
        # Phase 1: Foundation
        _show_section_header("Chapter 1: The Kennel", "Foundation & Workspace Settings")
        ...
        _show_section_header("Chapter 3: The Pack", "Optional Integrations (Telegram, WhatsApp, etc.)")
        if _get_questionary().confirm("Would you like to configure Chat Channels now?", default=False).ask():
            _configure_channels(config)
            
        # Final Review
        _show_summary(config)
        if _get_questionary().confirm("Save this configuration?", default=True).ask():
             _show_success_splash(get_config_path())
             return OnboardResult(config=config, should_save=True)
        else:
            console.print("[yellow]Changes not saved. Entering main menu for manual adjustment...[/yellow]")
            _get_questionary().press_any_key_to_continue().ask()

    # Main Menu Loop (for manual edits or non-fresh starts)
    while True:
        console.clear()
        ...
        if answer == "[W] Run Guided Wizard":
            # Just re-run the wizard logic (simplified)
            _configure_general_settings(config, "Agent Settings")
            _configure_providers(config)
            _configure_channels(config)
            continue
        ...
        _MENU_DISPATCH = {
            "[P] LLM Providers": lambda: _configure_providers(config),
            "[C] Chat Channels": lambda: _configure_channels(config),
            ...
            "[V] View Summary": lambda: _show_summary(config),
        }
