...
- **⚡ WebUI ultra-rapide & UX soignée** — Migration du frontend vers une architecture ES6 entièrement groupée via `esbuild` pour un chargement instantané, et amélioration de l'UX des Connected Apps pour permettre une configuration backend Klavis fluide sans rechargement manuel."""
    ...
    "README.ja.md": {
        "sync_from": "v0.9.4",
        "sync_to": "v0.9.5",
        "summary": "📢 <b>最新リリース: v0.9.5 (Big Release & Refactor)</b> — クリックで変更点を表示",
        "link": "完全なリリース履歴については [Changelog](./CHANGELOG.md) を参照してください。",
        "points": """- **🔥 Big Security Refactor & 🔐 暗号化された認証情報ボルト** — セキュリティを全面的に刷新しました。ShibaClaw は、API キー、ボットトークン、パスワードを安全に保存するために、堅牢な AES-128/256 対称暗号化ボルト（`credentials.enc` および `credentials.key`）を使用するようになりました。ボルト優先の解決、スレッドセーフ、破損防止保護、および厳密な OS レベルのアクセス許可（Unix では `0o600`、Windows では `icacls` ACL）を備えています。
...
- **⚡ WebUI ultrarrápida e UX aprimorada** — O frontend foi migrado para uma arquitetura ES6 totalmente empacotada via `esbuild` para carregamento instantâneo, e a UX dos Connected Apps foi suavizada para permitir a configuração contínua do backend Klavis sem recarregamentos manuais."""
    ...
    "README.zh-CN.md": {
        "sync_from": "v0.9.4",
        "sync_to": "v0.9.5",
        "summary": "📢 <b>最新版本：v0.9.5 (Big Release & Refactor)</b> —— 点击查看更新内容",
        "link": "完整版本历史请查看 [Changelog](./CHANGELOG.md)。",
        "points": """- **🔥 Big Security Refactor & 🔐 加密凭证保管库** —— 我们全面升级了安全性。ShibaClaw 现在使用强大的 AES-128/256 对称加密保管库（`credentials.enc` 和 `credentials.key`）来安全存储 API 密钥、Bot Token 和密码。它具有优先使用保管库、线程安全、防损坏保护以及严格的操作系统级权限控制（Unix 上为 `0o600`，Windows 上为 `icacls` ACL）。
