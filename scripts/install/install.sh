...
if ! command -v python3 &> /dev/null; then
    ...
    if [[ "$OSTYPE" == "linux-gnu"* ]]; then
        if command -v apt-get &> /dev/null; then
            ...
            echo "⚠️  Unsupported Linux distro for auto-install. Please install Python 3.12+ and python3-venv manually."
            exit 1
        fi
        ...
        if command -v brew &> /dev/null; then
            ...
            echo "⚠️  Homebrew not found. Please install Python 3.12+ manually or install Homebrew first."
            exit 1
        fi
    else
...
if command -v pipx &> /dev/null; then
    ...
    if [ -w "/usr/local/bin" ]; then
        ...
        echo "⚠️  Could not write to /usr/local/bin. You can run shibaclaw using: $VENV_DIR/bin/shibaclaw"
